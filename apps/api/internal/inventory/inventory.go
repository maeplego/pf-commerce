...
type Repository interface {
	...
	// AdjustReserved atomically adds deltaReserved if available (qty-reserved) >= need.
	// need is the extra units required when deltaReserved > 0.
	AdjustOnHand(ctx context.Context, siteID, productID string, deltaQty int, now time.Time) (Balance, error)
	TryReserve(ctx context.Context, siteID, productID string, qty int, now time.Time) (Balance, error)
...
func (s *Service) Reserve(ctx context.Context, siteID, productID, orderID, actorID string, qty int) (Reservation, error) {
	...
	if _, err := s.repo.TryReserve(ctx, siteID, productID, qty, now); err != nil {
		return Reservation{}, err
	}
	res := Reservation{
		ID:        id.New(),
